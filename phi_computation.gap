...
carat_folder := "/home/alexander/toric_ZG_module/ZG_module/carat_tables";
files := [Concatenation(carat_folder, "/cryst1.txt"),
          ...
          "/home/alexander/toric_ZG_module/ZG_module/external_gap_code/FlabbyResolution.gap"
];
...
CocycleRelation := function(gens, d, w)
  ...
  for i in Reversed(w{[1..Length(w)-1]}) do
    proj_ := SparseMatrix(proj_w_inverse(i));
    action_ := SparseMatrix(action(i));
    suka:= action_*result;
    Print("suka\n");
    Print(suka);
    Print("\nsuka\n");
...
Mod2Z1 := function(gens, relations)
  ...
  local c_rel, result, d, sparse_gens;
...
Z1 := function(gens)
  ...
  local gen, b, smith_form, s, p, _rank, d;
  d := DimensionsMat(gens[1])[1];
  b := [];
  if gens=[] then
      return [];
...
ComputePhi := function(gens, cr)
  local ZP, ZM, PM, ZPZM, BMZM, ZM_rank, HP_im, result, BM_rank, ZL2N, L2N,
        L2N2, mapk, ZL2N2, HL2N2_im, HL2N_im, relations;
